...
function App() {
  const [selectedComparator, setSelectedComparator] =
    useState<Comparator>(defaultComparator);
  ...
  const [comparatorDimensions, setComparatorDimensions] = useState(
    defaultComparator.dimensions
  );
  ...
  const handleComparatorChange = (newComparator: Comparator) => {
    ...
    if (
      newComparator.allowedPositions &&
      !newComparator.allowedPositions.includes(targetPosition)
    ) {
    ...
    <div className="h-screen bg-gray-100 relative">
      ...
      >
        <div className="w-6 h-6 flex flex-col justify-center space-y-1">
          <div
            className={`h-0.5 bg-gray-600 transition-transform ${
              isSidebarOpen ? "rotate-45 translate-y-1" : ""
            }`}
          ></div>
          <div
            className={`h-0.5 bg-gray-600 transition-opacity ${
